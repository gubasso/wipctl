...
- breaking, `record`: Name the session on an entry and the paths it changes ([#54](https://github.com/gubasso/wipctl/pull/54)). Every lane entry now requires `touches`, so an adopter adds `touches: []`, or a list of at most 12 unique exact paths, to every lane entry before validating and to every pending fragment before draining. Each path is relative, resolves against the host's project root, and carries no dot segment, no whitespace, and no glob character. Every path a story's `Amends` section names must appear in its entry's list. An entry in `doing` or `review` may also carry `session` as one `<alias>#<key>` token, whose alias names a table in the new optional `session_adapters` section of `.wipctl/plan.toml`. Each adapter table declares at least one of `inspect`, `contact`, and `resume` as an argument vector carrying exactly one `{key}` item, and an unknown operation key is rejected.
- breaking, `ranking`: Expose only declared preferences ([#51](https://github.com/gubasso/wipctl/pull/51)). An adopter replaces `[ranking].keys` with `[ranking].preferences`, containing only `delay-cost` and `points-ascending` in the chosen order. The tool fixes eligibility, dependency order, and the final ID tie-break outside configuration. An adopter replaces `class: expedite` with `delay_cost: immediate`, replaces `class: intangible` with `delay_cost: deferred`, and removes `class: standard` because absence means ordinary work.
- `documents`: Record the option a story refused and why it lost ([#53](https://github.com/gubasso/wipctl/pull/53))
