...
1. Confirm the root. `.wipctl.toml` sits at the project root, and its `project_id` agrees with the plan repository it resolves to. A disagreement fails validation naming both sides.
