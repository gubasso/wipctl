...
3. Write `Governed by`. State the claim, not the coordinates: link to a stable heading anchor, never a line number; name the page that owns a rule rather than eleven records that shaped it; write `None.` when genuinely nothing governs.
4. Write `Amends`. The three gated rules: the path is relative; it resolves against the root unless the assertion opens with `new:`; only the leading inline-code token is the path. Write `None` when the work changes no durable document.
...
Review-only responsibilities, named because no gate holds them: the named document is the right one; the transfer happened; a `new:` promise was kept; `Governed by` names the sources a session genuinely needs; a reference genuinely governs or is genuinely changed.
