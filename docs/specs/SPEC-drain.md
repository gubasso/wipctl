...
The drain MUST write fragments into each lane file ascending by the stated capture instant, with a residual tie on the full id compared lexically as one string.

#### Scenario: Two machines drain the same fragments

- GIVEN one set of fragments on two machines
- WHEN each drains
- THEN the resulting lane-file bytes agree, because the byte-order key is total and nothing parses an id into parts
