...
The writers — `fix` (rank order), `move` and `start` (transitions), `land` (fragments), `delete` (removals), `new` (captures), `rename` (identity), `sync` and `resolve` (reconciliation) — are enumerated with the lock and the commit rules in [../cli/conventions.md](../cli/conventions.md). Everything any of them writes is bound by the same discipline.
...
The four bind `fix` and the rank-repair phase `move` and `start` run (ADR-0010). The other writers are invoked to change a legal record; what every writer owes on the bytes it touches is the shared discipline below.
...
- The writer lock MUST be held for the whole transaction — from the deciding read to the plan trunk commit, the rank repair included; see [../cli/conventions.md](../cli/conventions.md).
...
Only the rank repair MAY reorder, and three verbs invoke it: `fix` on explicit request, and `move` and `start` after the bottom-of-lane insertion. `land` deliberately runs no repair, and MUST report any resulting ranking failure, leaving the repair as a separate act, because a landing that silently reorders is a machine making a rank claim. `delete` removes and MUST NOT reorder.
