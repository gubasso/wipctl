...
At most one positional (else exit 2 `next takes at most one plan directory`). No flags, and no `--json` by design: the line is already the machine format.
...
- Prints exactly one line on stdout: the head entry of `doing.yml`, or of `todo.yml` when `doing` is empty:
  ...
  The title is the story document's H1 with its leading `<id> —` prefix stripped; when the document is unreadable the id MUST stand in.
...
- `next` MUST NOT gate. A record defect MUST be emitted on stderr as `wipctl: warning: <location>: <message>` and the line printed anyway. A head that is not startable — blocked by an open question or an unclosed need, whether from an R1 violation or a lane holding no eligible entry at all — MUST be warned about, not fatal: `wipctl: warning: rate-limit-the-search-endpoint-a7f3 is blocked by an open question`.
