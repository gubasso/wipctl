...
Capture: mints the identity, writes the story document and the pending fragment, touches no lane file. Two sessions in two clones capture at the same moment and cannot collide on purpose, because nobody asks what the next id is.
...
- Mint: the title's slug (lowercase kebab; punctuation, capitals, and runs of spaces normalised) plus four lowercase hex characters from a source of randomness. A uid collision with an existing id MUST be re-minted, never incremented — an incremented uid is a counter.
...
- Write discipline inherited from `init`: every destination MUST be checked before the first byte; an existing path MUST be reported and left alone; no overwrite flag exists. The `pending/` directory MUST be created when absent. A `--epic` naming no document MUST be refused before any write.
