...
Four panels, one screen, each titled by the reader's question. Pure composition: it calls the verbs that own each panel and computes nothing itself, so a number can never appear in two places with two values.
