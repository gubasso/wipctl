...
- One id per line on stdout: entry ids from all five lanes in record order, then epic ids from the
  documents, then question ids — one shared namespace, one stream. No header, no alignment, no
  sorting.
- The verb MUST NOT gate: a record the checker would reject still yields the ids it could read,
  exit 0. An empty record prints nothing, exits 0.
