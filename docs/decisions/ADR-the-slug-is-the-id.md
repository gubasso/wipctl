# The slug is the id; nothing random is minted

## Context and Problem Statement

The four-hex suffix existed for exactly one reason: allocation was unobservable. Two clones with no shared state could not ask each other what the next id was, so the mint drew randomness and collisions were surfaced rather than prevented. With one live record behind one machine-level lock (ADR-the-plan-record-lives-in-its-own-repository, ADR-the-writer-lock-is-per-project-per-machine), a mint can read the record it is about to write into, and the reason is gone.
...
Supersedes ADR-the-program-mints-the-identity. Re-scopes ADR-a-counter-is-forbidden-where-allocation-is-concurrent and ADR-capture-disjointness-holds-outside-collision; amends ADR-the-drain-orders-by-the-stated-instant.

Amended by ADR-a-qualifying-postfix-is-part-of-the-id — a qualifying postfix is part of the id it disambiguates, not a departure from the slug.
