# ADR-0039: The repository is self-contained
...
Chosen option: `hold every load-bearing fact in-repo` — the repository states what it depends on, so a clone on a machine with nothing else still explains itself, builds, and gates. Borrowed substance is rewritten into the zone that owns it rather than referenced, which also forces it through the language-agnostic constraint the reference carries. Vendoring was rejected because a copied document arrives with a second voice, its own history, and no owner here, and it decays without anyone noticing. The rule and its test are stated in `docs/reference/quality-gates.md`; `AGENTS.md` points at that statement for every agent entering the repository.
