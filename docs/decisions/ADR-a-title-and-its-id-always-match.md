# A title and its id always match; renaming is an operation

## Context and Problem Statement

With the slug as the whole id (ADR-the-slug-is-the-id), an id is derived from its title — and a title edited after capture would silently disagree with it. A drift a reader has to tolerate, or an invariant a tool holds: the record must pick one.
...
Amends ADR-a-deletion-is-recorded-and-burns-the-id.

Amended by ADR-a-qualifying-postfix-is-part-of-the-id — an id is its title's slug plus at most a qualifying postfix, and a rename that frees no id burns nothing.
