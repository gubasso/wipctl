# A counter is forbidden where allocation is concurrent
...
Amended by ADR-capture-disjointness-holds-outside-collision — the gate names claimants that coexist in one record; a same-path collision between clones surfaces as a version-control conflict, resolved by re-minting one capture whole.

Amended by ADR-the-slug-is-the-id — a qualifying postfix is legal where the lock makes allocation serial; between machines allocation stays concurrent and a collision is surfaced, never prevented.
