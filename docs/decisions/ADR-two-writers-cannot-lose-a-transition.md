# Two writers cannot lose a transition
...
Superseded by ADR-the-writer-lock-is-per-project-per-machine — the writer lock is per project per machine and protects a transaction.
