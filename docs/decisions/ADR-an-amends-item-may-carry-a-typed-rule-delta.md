# An Amends item may carry a typed rule delta
...
The gate checks the clause's shape when one is present and never its presence: the program cannot know which host documents are specifications, and asserting nothing about the host is ADR-the-host-stays-unassumed. Whether a story that changed a rule declared the clause is a review responsibility, stated as such.
