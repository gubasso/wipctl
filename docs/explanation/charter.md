...
- Every public name explains itself. A domain, concept, field, or field value uses one lean, plain-language name. The owning page defines it without requiring knowledge of another method.
...
- No priority a machine authors. The tool applies fixed constraints, then the plan's declared preferences. The full id resolves only entries whose facts express no preference.
